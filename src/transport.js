...
/**
 * HTTP transport. Subclass (or duck-type) postForm/get to inject a fake in
 * tests. Responses are plain objects:
 *   { status, body (Buffer|string), headers (lowercased map), error? }
 */
class FetchHttpClient {
  ...
  constructor(options = {}) {
    ...
    this.userAgent = options.userAgent ?? 'systemlocker-bedrock-node/1.2.0';
  }
...
async function readBoundedBody(response, controller) {
  ...
  if (Number.isFinite(length) && length > MAX_RESPONSE_BYTES) {
    controller.abort();
    throw new Error('response body exceeds 1 MiB limit');
  }
...
function headerOf(httpResponse, name) {
  return httpResponse.headers[name.toLowerCase()] ?? '';
}

module.exports = { FetchHttpClient, responseOk, headerOf };
